...
export class AuthController {
    ...
    public async logIn(@Body() userDto: AuthDto): Promise<string> {
        // Corregido: Usamos userDto en lugar de auth
        const { username, password } = userDto;

        // Creamos un payload básico para firmar el token por ahora
        const payload = { username: username };

        // Corregido: Pasamos el payload y agregamos async a la función
